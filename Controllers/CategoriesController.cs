...
namespace OnlineClothStore.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public ActionResult Index(string search, int? page)
        {
            ...
            {
                // Perform a search operation based on the 'search' parameter
                // For example, you can use Entity Framework to query your database
                // and return the filtered results.
                var category = db.Categories
                    .Where(c => c.Category_Name.StartsWith(search))
                    .ToList();

                var pagedData = category.ToPagedList(pageNumber, pageSize);
                return View(pagedData);
            }

            // If no search query provided, you can return the complete list of categories.
           else
            {
